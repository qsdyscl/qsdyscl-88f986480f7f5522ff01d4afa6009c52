...
def file2matrix(path, delimiter):
    ...
    # 逐行遍历，结果按分隔符分隔为行向量
    recordlist = [list(map(eval, row.decode().split(delimiter))) for row in rowlist if row.strip()]
    # 返回转换后的矩阵形式
...
# 绘制散点图
def drawScatter(k, plt, mydata, size=20, color='blue', mrkr='o'):
    x = [mydata[i, 0] for i in range(k)]
    y = [mydata[i, 1] for i in range(k)]
    plt.scatter(x, y, s=size, c=color, marker=mrkr)
